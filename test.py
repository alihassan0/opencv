...
def resize(imagePath, width, height):
    ...
    while xPos < width: #Loop through rows
        while yPos < height: #Loop through collumns
            valR = sourceImage.item(int(yPos / height * sourceImage.shape[0]), \
                                    int(xPos / width * sourceImage.shape[1]),2)
            valG = sourceImage.item(int(yPos / height * sourceImage.shape[0]), \
                                    int(xPos / width * sourceImage.shape[1]),1)
            valB = sourceImage.item(int(yPos / height * sourceImage.shape[0]), \
                                    int(xPos / width * sourceImage.shape[1]),0)
            print(val, int(yPos / height * sourceImage.shape[0]), int(xPos / width * sourceImage.shape[1]))
            blankImage.itemset((yPos, xPos, 0), val) #Set B to val
            blankImage.itemset((yPos, xPos, 1), val) #Set G to val
            blankImage.itemset((yPos, xPos, 2), val) #Set R to val
...
resize("images/logo.jpg", 669.0, 325.0)
